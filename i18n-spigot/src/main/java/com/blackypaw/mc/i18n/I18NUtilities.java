...
import com.blackypaw.mc.i18n.interceptors.*;
...
public class I18NUtilities extends JavaPlugin {
	...
	public void onEnable() {
		...
		this.installInterceptor(new ChatInterceptor(this));
		this.installInterceptor(new ScoreboardInterceptor(this));
		this.installInterceptor(new SettingsInterceptor(this));
		this.installInterceptor(new SignInterceptor(this));
		this.installInterceptor(new TitleInterceptor(this));
		this.installInterceptor(new SlotInterceptor(this));
	}
	...
	/**
	 * Installs the necessary packet interceptors required for intercepting packets containing text to be
	 * translated.
	 */
	private void installInterceptor(I18NInterceptor interceptor) {
		final ProtocolManager protocolManager = ProtocolLibrary.getProtocolManager();

		for (final I18NHook hook : interceptor.hooks()) {
			protocolManager.addPacketListener(new PacketAdapter(this, ListenerPriority.LOWEST, hook.getPacketType()) {
				@Override
	...
	public String restoreTextFromChatComponent(WrappedChatComponent component) {
		return this.gson.fromJson( component.getJson(), ChatComponent.class ).getUnformattedText();
	}

	public String translateMessageIfAppropriate(Locale locale, String message) {
